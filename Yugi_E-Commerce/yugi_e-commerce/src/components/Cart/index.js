...
const Cart = () => {
    ...
    if(cart.value == 0){
        localStorage.setItem('Yugi E-commerce: cart', JSON.stringify(cart))
    }

    return(
        <>
            <button type="button" className="btn btn-warning" data-bs-toggle="modal" data-bs-target="#CartModal">
                <span><i className="fas fa-shopping-cart"></i></span>
                <span className="badge rounded-pill btn-warning text-dark">
                    {cart.value}
                </span>
            </button>
            ...
            <div className="modal fade" id="CartModal" tabIndex="-1" aria-labelledby="CartModalLabel" aria-hidden="true">
                <div className="modal-dialog modal-xl">
                    ...
                    <h5 className="modal-title" id="CartModalLabel">Meu Carrinho</h5>
                        <button type="button" className="badge btn-warning" data-bs-dismiss="modal" aria-label="Close">
                            ...
                            {cart.Cart.map( item =>{
                                return(
                                    <tr class="align-middle" key={item.id}>
                                        ...
                                        <th><span className="badge badge-pill bg-primary">
                                            {item.quantity}
                                        </span></th>
                                        ...
                                        <th><button onClick={()=>dispatch(cartActions.AddItem(cart, item))} className="badge badge-pill bg-success"><i className="fas fa-plus"></i></button></th>
